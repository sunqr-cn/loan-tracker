  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* 月供卡片 - 主色 */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 p-5 text-white shadow-lg shadow-blue-600/20">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-medium text-blue-50">当前月供</span>
            </div>
            <div className="text-3xl font-bold tracking-tight">¥{formatMoney(currentMonthly)}</div>
        ...
        {/* 剩余本金卡片 */}
        <div className="relative overflow-hidden rounded-2xl bg-white p-5 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-lg bg-amber-50 flex items-center justify-center">
              <PiggyBank className="w-4 h-4 text-amber-500" />
            </div>
            <span className="text-xs font-medium text-gray-400">剩余本金</span>
          </div>
          <div className="text-3xl font-bold tracking-tight text-gray-800">¥{formatMoney(remainingPrincipal)}</div>
          <div className="mt-2 flex items-center gap-1.5 text-xs text-gray-400">
            <span>贷款总额 ¥{formatMoney(loanInfo.totalAmount)}</span>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl bg-white p-5 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-medium text-gray-400">累计利息</span>
          </div>
          <div className="text-3xl font-bold tracking-tight text-orange-500">¥{formatMoney(totalInterest)}</div>
          <div className="mt-2 flex items-center gap-1.5 text-xs text-gray-400">
            <span>占还款总额 {interestRatio.toFixed(1)}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex flex-col items-center justify-center">
          <ProgressRing progress={progress} paid={paidCount} total={schedule.length} />
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm lg:col-span-2">
          ...
          <div className="h-3 bg-gray-100 rounded-full overflow-hidden mb-4">
            <div className="h-full bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 rounded-full transition-all duration-700"
              style={{ width: `${progress}%` }} />
          </div>
          <div className="grid grid-cols-4 gap-3">
            <StatBox label="已还期数" value={`${paidCount}`} unit="期" color="text-green-600" />
            <StatBox label="剩余期数" value={`${unpaidCount}`} unit="期" color="text-gray-600" />
            <StatBox label="还款总额" value={`¥${formatMoney(totalPayment)}`} unit="" color="text-blue-600" />
            <StatBox label="已还本金" value={`¥${formatMoney(totalPrincipal - remainingPrincipal)}`} unit="" color="text-indigo-600" />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {prepayments.length > 0 && (
            <button onClick={() => setActiveTab('config')}
              className="group bg-white rounded-2xl p-5 border border-gray-100 shadow-sm text-left hover:border-blue-300 hover:shadow-md transition-all">
              ...
              <div className="flex items-end gap-4">
                <div>
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">笔记录</div>
                </div>
                <div className="flex-1 text-right">
                  <div className="text-sm font-medium text-gray-600">¥{formatMoney(prepayments.reduce((s, p) => s + p.amount, 0))}</div>
                  <div className="text-xs text-gray-400 mt-0.5">合计金额</div>
                </div>
              </div>
            </button>
          )}
          {rateChanges.length > 0 && (
            <button onClick={() => setActiveTab('config')}
              className="group bg-white rounded-2xl p-5 border border-gray-100 shadow-sm text-left hover:border-indigo-300 hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
              ...
              <div className="flex items-end gap-4">
                <div>
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">次调整</div>
                </div>
                <div className="flex-1 text-right">
                  <div className="text-sm font-medium text-gray-600">
                    {rateChanges[0].oldRate}% → {rateChanges[rateChanges.length - 1].newRate}%
                  </div>
                  <div className="text-xs text-gray-400 mt-0.5">利率变化</div>
                </div>
              </div>
            </button>
          )}
...
function StatBox({ label, value, unit, color }: { label: string; value: string; unit: string; color: string }) {
  return (
    <div>
      ...
      <div className={`text-sm font-bold ${color}`}>{value}<span className="text-xs ml-0.5 text-gray-400">{unit}</span></div>
    </div>
  );
}

function ProgressRing({ progress, paid, total }: { progress: number; paid: number; total: number }) {
  const radius = 52;
  ...
  return (
    <div className="relative w-36 h-36 flex items-center justify-center">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <div className="text-xs text-gray-400 mt-0.5">{paid}/{total} 期</div>
      </div>
    </div>
  );
}
